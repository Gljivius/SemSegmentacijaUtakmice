...
RGB_DIR = r'C:\Users\6lj1v\Desktop\SemSegmentacija\Slike\SlikeZaUcenje'      #unesite ovdje svoj put do datoteka
GT_DIR = r'C:\Users\6lj1v\Desktop\SemSegmentacija\Slike\SlikeOznaka'         #unesite ovdje svoj put do datoteka
OUTPUT_DIR = r'C:\Users\6lj1v\Desktop\SemSegmentacija\Slike\IzlazneSlike'    #unesite ovdje svoj put do datoteka
IMG_WIDTH = 1280
IMG_HEIGHT = 738
...
class Trainer(object):
  ...
  def add_image(self, filename):
    ...
    rgb_img = ski.transform.resize(rgb_img, (height, width), mode = 'constant', preserve_range=True, order=3)
    rgb_img = rgb_img.astype(np.float32)
    for c in range(3):
      rgb_img[:,:,c] -= VGG_MEAN[c]
    rgb_img = rgb_img.reshape(1, height, width, 3)
    labels = ski.data.load(label_path)
    labels = ski.transform.resize(labels, (height, width), mode = 'constant', preserve_range=True, order=0)
